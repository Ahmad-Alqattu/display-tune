...
const DEBUG = !!GLib.getenv('DISPLAY_TUNE_SAT_DEBUG');
...
class SaturationEffect extends Shell.GLSLEffect {
    ...
    constructor(params) {
        ...
        if (DEBUG) {
            this._debugId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 2, () => {
                console.log(`display-tune-saturation: paints=${this._paints ?? 0} stale-cache-avoided=${this._staleCacheAvoided ?? 0}`);
    ...
    // With several monitors each stage view paints on its own clock. Clutter clears the
    // actor's dirty state after the first view paints, so the next view would reuse the
    // cached FBO, which only holds the first view's clip region -> flicker ("dancing").
    // Treating the actor as dirty on every paint makes each view re-render its own region.
    vfunc_paint(node, paintContext, flags) {
        if (DEBUG) {
            this._paints = (this._paints ?? 0) + 1;
            if (!(flags & Clutter.EffectPaintFlags.ACTOR_DIRTY))
                this._staleCacheAvoided = (this._staleCacheAvoided ?? 0) + 1;
        }
